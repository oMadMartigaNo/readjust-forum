...
/* change log:
 * Fri Mar 15 00:34:23 GMT 2013: 0.1.4b
 * - added PATH_ROOT to Origional files
 * Fri Mar 15 23:43:01 GMT 2013: 0.1.5b
 * - added ExpireDeadCacheFiles
 * - external JS defer sort before local
 * - clear cache button
 */
...
class Consolidate extends Gdn_Plugin {
   ...
   protected function ChunkedFiles($Put = array(),$Save = TRUE){
	   $Token = 'chunked_files';
	   $CacheFile = PATH_LOCAL_CACHE."/Consolidate/$Token";
	   //check cache
   ...
   public function SettingsController_Consolidate_Create($Sender,$Args){
		...
		if($Sender->Form->IsPostBack() != False){
			...
			foreach($FormValues['Chunks'] As $ChunkGroupIndex => $Chunk){
				...
				if(!is_string($Ext) || preg_match('`^\.[a-z0-9]+$`I','',$Ext))
					continue;
				
				if(!GetValue($Ext,$ChunkGroups))
					$ChunkGroups[$Ext] = array();
					
				$ChunkGroups[$Ext][] = trim($Chunk);
			}
   ...
   public function HeadModule_BeforeToString_Handler($Head) {
	  ...
	  foreach($ChunkedJs As $JsChunkGroup => $JsChunk){
		...
		if(!C('Plugins.Consolidate.DeferJs')){
			$Head->AddScript("/cache/Consolidate/$Token", 'text/javascript', FALSE);
		}else{
			$this->DeferJs[]=$Token;
		}
	  }	  
   ...
   public function Base_AfterBody_Handler($Sender){
	   if(!empty($this->DeferJs)){
		   ...
		   foreach($this->DeferJs As $Token){
			   $SortLast++;
			   $Head->AddScript("/cache/Consolidate/$Token", 'text/javascript',$SortLast);
		   }
		   $Tags = $Head->GetTags();
