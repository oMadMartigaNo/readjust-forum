...
//echo 'dmodel';
	...
	//	  echo 'pcheck';
      ...
      $Url = $DiscussionID . '/comment/' . $Sender->EventArguments['Object']->CommentID;

    }
	$InsertID = $Data->InsertUserID;
  //echo "The InsertID for this post is ".$InsertID."The UserID is".$UID;
	if($InsertID == $UID)
	  $Self = TRUE;
  ...
  public function FormatLikes($Likes, $Url, $UID, $Self) {
	...
	if(!$Self){
	  ...
	  else 
	  {
        ...
        $QuoteText = T('<button class="btn btn-primary" type="button"><i class="icon-thumbs-up icon-white"></i>Like</button>');
	  }
